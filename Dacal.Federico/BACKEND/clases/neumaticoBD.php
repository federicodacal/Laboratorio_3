...
class NeumaticoBD extends Neumatico implements IParte1, IParte2, IParte3, IParte4
{
    ...
    public static function traerPorMarcaYMedidas(string $marca, string $medidas) : NeumaticoBD | null
    {
        $neumatico = null;
        ...
        if(isset($marca) && isset($medidas))
        {
            ...
            $consulta = $objetoAccesoDato->retornarConsulta("SELECT * FROM neumaticos WHERE marca = :marca AND medidas = :medidas");
    
            $consulta->bindValue(":marca", $marca, PDO::PARAM_STR);
            $consulta->bindValue(":medidas", $medidas, PDO::PARAM_STR);
    ...
    public static function mostrarFotosModificadosFRONT() : string 
    {
        $files = array();
        ...
        foreach (scandir('./neumaticosModificados') as $file) 
        {
            if ($file !== '.' && $file !== '..') 
            {
                ...
                $mensaje .= "<tr>";
                $mensaje .= "<td>" . $path . "</td>";
                $mensaje .= "<td><img src='./BACKEND" . $path . "' alt='Nope' width='50px' height='50px'></td>";
                $mensaje .= "</tr>"; 
            }
        }
        return $mensaje;
